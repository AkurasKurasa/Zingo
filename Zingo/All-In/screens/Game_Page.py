...
class GamePage(Screen):
    ...
    def next_question(self) -> None:
        ...
        # Guard: no questions
        if not getattr(self.app, 'QUESTIONS', None) or len(self.app.QUESTIONS) == 0:
        ...
        # Flexible field getter (dict or dataclass)
    ...
    def _process_correct(self):
        print("Correct")
        # result = self.zingo_engine.run_zingo("increment_state")
        # print(result)
        ...
        if self.app.POINTS >= self.app.REQUIRED_POINTS:
            ...
            self.flash_label.opacity = 1
            self.confetti.burst(count=150, center=(self.layout.width * 0.5, self.layout.height * 0.05))
            self.back_btn.opacity = 1
            self.back_btn.disabled = False
            self.middle_layout.disabled = True
            return

        # Roulette at 5 in a row
    ...
    def _process_incorrect(self):
        print("Wrong")
        self.app.QUESTIONS_IN_A_ROW = 0
        self.streak_label.text = "Streak: 0"  # ← RESET
